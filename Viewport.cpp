...
void  Viewport::InitGraphics()
{
    ...
    deviceContext->Map(pVBuffer, 0, D3D11_MAP_WRITE_DISCARD, 0, &ms);    // map the buffer
    memcpy(ms.pData, OurVertices, sizeof(OurVertices));                 // copy the data
    deviceContext->Unmap(pVBuffer, 0);                                      // unmap the buffer
}

// this function loads and prepares the shaders
void  Viewport::InitPipeline()
{
    // load and compile the two shaders
    ID3D10Blob *VS, *PS;

    D3DCompileFromFile("../shaders.shader", 0, 0, "VShader", "vs_4_0", 0, 0, &VS, 0);
    D3DCompileFromFile("../shaders.shader", 0, 0, "PShader", "ps_4_0", 0, 0, &PS, 0);
